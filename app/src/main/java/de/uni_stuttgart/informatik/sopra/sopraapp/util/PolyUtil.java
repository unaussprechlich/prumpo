...
public class PolyUtil {
     ...
     */
    public static boolean intersects(double lat1, double lat2, double lng2,
        ...
        if (lat1 <= -PI/2 || lat2 <= -PI/2 || lat1 >= PI/2 || lat2 >= PI/2) return false;
    ...
    public static boolean containsLocation(double latitude, double longitude, List<LatLng> polygon, boolean geodesic) {
        ...
        double pointLat = toRadians(latitude);
        double pointLng = toRadians(longitude);
        ...
        for (LatLng p : polygon) {
            ...
            double dLng3 = wrap(pointLng - lng1, -PI, PI);

            // Special case: point equal to vertex is inside.
            if (pointLat == lat1 && dLng3 == 0) {
                return true;
            }
            double pLat = toRadians(p.latitude);
            double pLng = toRadians(p.longitude);

            // Offset longitudes by -lng1.
            if (intersects(lat1, pLat, wrap(pLng - lng1, -PI, PI), pointLat, dLng3, geodesic)) {
